...
namespace ExpertoLib {
	...
	void ExpertoDifuso::testDifuso() {
		...
		for (int i = 0; i <= 49; ++i) {
			scalar carga = Load->getMinimum() + i * (Load->range() / 50);
			Load->setValue(carga);
			scalar suciedad = Dirt->getMinimum() + i * (Dirt->range() / 50);
			Dirt->setValue(suciedad);
			engine->process();
			std::cout << "carga(input) = " << Op::str(carga) << " => " << ", suciedad(input) = " << Op::str(suciedad) <<
				" entonces Detergente = " << Op::str(Detergent->getValue()) << std::endl;
			//FL_LOG("obstacle.input = " << Op::str(location) << " => " << "steer.output = " << Op::str(mSteer->getValue()));
		}
		std::cout << "desde difuso\n";
	}
